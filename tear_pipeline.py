...
A config template can be generated with:
    python tear_pipeline.py --write-template config.txt
"""
...
# Optional cleaning of Inpactor2 library before reannotation
...
def main():
    ...
    parser.add_argument("--clean-inpactor-lib", action="store_true", help="Clean Inpactor2 library before reannotation")
    ...
    if args.plot:
        params["plot"] = "true"
    if args.clean_inpactor_lib:
        params["clean_inpactor_lib"] = "true"
    ...
    outdir = Path(get_param(params, "outdir", default="TEAR_RESULTS")).expanduser().resolve()
    ...
    clean_inpactor_lib = get_param(params, "clean_inpactor_lib", default=False, cast=bool)
    clean_inpactor_script_name = get_param(params, "clean_inpactor_script", default="count_ltr_classif_valeurextreme_cleaning.py")
    clean_inpactor_output_name = get_param(params, "clean_inpactor_output", default="inpactor2_library.cleaned.fa")
    ...
    final_prefix = get_param(params, "final_prefix", default="tear")
    ...
    log_message("Starting TEAR annotation refinement pipeline", log_file)
    ...
    # Helper scripts
    script_length = scripts_dir / get_param(params, "script_min_length", default="filter_repeatmasker_min_length.py")
    script_simple = scripts_dir / get_param(params, "script_simple_repeat", default="filter_repeatmasker_simple_repeat.py")
    script_overlap = scripts_dir / get_param(params, "script_count_overlaps", default="count_TE_overlaps_out.py")
    script_score = scripts_dir / get_param(params, "script_score_filter", default="filter_annotation_score_out.py")
    script_reannot = scripts_dir / get_param(params, "script_reannot", default="reannotate_ltr_inpactor2_then_tesorter_harmonized.py")
    script_tbl = scripts_dir / get_param(params, "script_tbl", default="rebuild_tbl_harmonized.py")
    script_gff = scripts_dir / get_param(params, "script_gff", default="repeatmasker_out_to_gff3.py")
    script_plot = scripts_dir / get_param(params, "script_plot", default="plot_te_gene_density_from_out.py")
    script_clean_inpactor = scripts_dir / clean_inpactor_script_name
    ...
    if clean_inpactor_lib:
        required_scripts.append(script_clean_inpactor)
    ...
    final_out = outdir / f"{final_prefix}.final.filtered.reannotated.out"
    final_tbl = outdir / f"{final_prefix}.final.repeatmasker_like.tbl"
    final_gff = outdir / f"{final_prefix}.final.repeatmasker.gff3"
    ...
    # Optional Step 6A: clean Inpactor2 library before reannotation
    inpactor_lib_for_reannotation = inpactor_lib
    if clean_inpactor_lib:
        clean_inpactor_output = Path(clean_inpactor_output_name)
        if not clean_inpactor_output.is_absolute():
            clean_inpactor_output = outdir / clean_inpactor_output

        run_cmd([
            sys.executable, script_clean_inpactor,
            "-i", inpactor_lib,
            "-o", clean_inpactor_output
        ], log_file, dry_run, "STEP 6A - Cleaning Inpactor2 library before reannotation")

        inpactor_lib_for_reannotation = str(clean_inpactor_output)
        log_message(f"STEP 6A done: cleaned Inpactor2 library written to {clean_inpactor_output}", log_file)
    else:
        log_message("STEP 6A skipped: clean_inpactor_lib=false", log_file)
    ...
    reannot_cmd = [
        ...
        "--rmout", step4,
        "--hite-lib", hite_lib,
        "--inpactor-lib", inpactor_lib_for_reannotation,
        "--out", step6,
        "--threads", threads,
    ...
    run_cmd(
        ...
        log_file,
        dry_run,
        "STEP 6B/8 - Reannotating LTR with Inpactor2 first, then optional TEsorter"
    )
    log_message(
        f"STEP 6B done: reannotation completed with min_identity={min_identity}, min_qcov={min_qcov}.",
        log_file
    )
