...
CLASS_ORDER = [
    ...
    "LTR/Copia",
    "LTR/Copia/unclassified",

    "LTR/ERV",

    "LTR/Gypsy",
    "LTR/Gypsy/unclassified",
...
def is_ltr_third_level(cls):
    """
    Détecte les annotations de 3e niveau :
    LTR/Gypsy/TAT
    LTR/Gypsy/REINA
    LTR/Copia/SIRE
    etc.
    ...
    return (
        len(parts) >= 3
        and parts[0] == "LTR"
        and parts[1] in {"Gypsy", "Copia"}
        and parts[2].lower() != "unclassified"
    )
...
def write_tbl(output, counts, lengths, total_annotated_bp, genome_size):
    ...
    ltr_copia_families = sorted(
        cls for cls in counts
        if cls.startswith("LTR/Copia/") and not cls.lower().endswith("/unclassified")
    )

    ltr_gypsy_families = sorted(
        cls for cls in counts
        if cls.startswith("LTR/Gypsy/") and not cls.lower().endswith("/unclassified")
    )
    ...
    with open(output, "w") as out:
        ...
        out.write("--------------------------------------------------------------------\n")
        out.write(f"{'Class':<32} {'Count':>8} {'bp masked':>16} {'% genome':>10}\n")
        out.write("--------------------------------------------------------------------\n")

        for cls in CLASS_ORDER:
            c = counts.get(cls, 0)
            bp = lengths.get(cls, 0)
            out.write(f"{cls:<32} {c:>8} {bp:>16} {pct_text(bp, genome_size):>10}\n")

            if cls == "LTR/Copia":
                for fam in ltr_copia_families:
                    c = counts.get(fam, 0)
                    bp = lengths.get(fam, 0)
                    out.write(f"  {fam:<30} {c:>8} {bp:>16} {pct_text(bp, genome_size):>10}\n")

            if cls == "LTR/Gypsy":
                for fam in ltr_gypsy_families:
                    c = counts.get(fam, 0)
                    bp = lengths.get(fam, 0)
                    out.write(f"  {fam:<30} {c:>8} {bp:>16} {pct_text(bp, genome_size):>10}\n")
        ...
        already_printed = set(CLASS_ORDER + ltr_copia_families + ltr_gypsy_families)

        for cls in sorted(counts):
            if cls not in already_printed:
                other_count += counts[cls]
                other_bp += lengths[cls]

        if other_count > 0:
            out.write(f"{'Other':<32} {other_count:>8} {other_bp:>16} {pct_text(other_bp, genome_size):>10}\n")

        out.write("--------------------------------------------------------------------\n\n")

        out.write("Summary groups\n")
        out.write("--------------------------------------------------------------------\n")
        out.write(f"{'Group':<32} {'Count':>8} {'bp masked':>16} {'% genome':>10}\n")
        out.write("--------------------------------------------------------------------\n")
